...
%%% ENGLISH
\englishAbstract{This thesis designs and evaluates \textLR{HAKE-MER} for multi-label emotion detection on GoEmotions. Under a fixed fine-tuning protocol (DistilBERT, three seeds; Step~0 also on RoBERTa), ablations H1--H4 show that label-specific phrase attention (M2) yields the main test macro F1 gain ($\approx 0.505$), hierarchical encoding alone (M1) underperforms a flat PLM baseline, and lexicon (M3) plus dynamic scaling (M4) do not reliably improve macro F1 over M1+M2. The recommended stack is M1+M2. Per-class analysis indicates gains mainly on mid-frequency emotions, not the rarest labels.}
