...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="M1+M2+M3+M4 campaign (H4, DistilBERT)")
    p.add_argument(
        "--lexicon",
        default="senticnet",
        choices=["nrc", "senticnet"],
        help="Lexicon for M3 (fixed while testing M4 dynamic scaling).",
    )
...
def main() -> None:
    ...
    report = {
        "backbone": args.backbone,
        "step": f"m1_m2_m3_{args.lexicon}_m4",
        "modules": {
            "use_m1": True,
            "use_m2": True,
            "use_m3": True,
    ...
    out_path = artifact_dir / f"m1_m2_m3_{args.lexicon}_m4_{slug}_campaign.json"
    ...
    print(
        f"M1+M2+M3+M4 ({args.lexicon}) done ({len(seeds)} seeds). "
