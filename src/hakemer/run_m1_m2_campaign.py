...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="M1+M2 campaign (H2, DistilBERT, 3 seeds)")
...
def main() -> None:
    ...
    report = {
        "backbone": args.backbone,
        "step": "m1_m2",
        "modules": {"use_m1": True, "use_m2": True, "use_m3": False, "use_m4": False},
        "protocol": {
            "epochs": args.epochs,
            "batch_size": args.batch_size,
            "lr": args.lr,
    ...
    out_path = artifact_dir / f"m1_m2_{slug}_campaign.json"
    out_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    agg = report["test_aggregate"]
    print(
        f"M1+M2 campaign done ({len(seeds)} seeds). "
